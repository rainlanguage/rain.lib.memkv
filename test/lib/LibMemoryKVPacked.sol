...
///
/// A `PackedMemoryKV` is a handle into shared memory, not a snapshot, although
...
/// `PackedMemoryKV` MUST NOT be returned from or passed into an external call. In
...
type PackedMemoryKV is uint256;

/// @dev The only valid starting value for a `PackedMemoryKV`. A store MUST begin here
/// and MUST only ever be advanced by `set`; any other value is undefined
/// behaviour.
PackedMemoryKV constant PACKED_MEMORY_KV_EMPTY = PackedMemoryKV.wrap(0);

/// @title LibMemoryKVPacked
/// The packed-handle layout, kept in the test tree as the oracle that
/// `LibMemoryKV.packedParity.t.sol` checks `LibMemoryKV` against. For any
/// sequence of `set` calls the two answer every `get` and `has` alike and
/// export the same words in the same order. Its 16 bit head pointers cap its
/// nodes at `0xFFFF`, so a parity test inserts into it before the store under
/// test allocates anything.
library LibMemoryKVPacked {
    ...
    /// The count is written by shifting into the top bits of `PackedMemoryKV`, so
    ...
    function get(PackedMemoryKV kv, MemoryKVKey key) internal pure returns (uint256 exists, MemoryKVVal value) {
    ...
    function has(PackedMemoryKV kv, MemoryKVKey key) internal pure returns (bool) {
    ...
    function set(PackedMemoryKV kv, MemoryKVKey key, MemoryKVVal value) internal pure returns (PackedMemoryKV) {
        uint256 pointer;
        uint256 length;
        assembly ("memory-safe") {
    ...
    function toBytes32Array(PackedMemoryKV kv) internal pure returns (bytes32[] memory array) {
